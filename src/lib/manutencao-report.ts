...
export type MaintenanceReportParams = {
  equipNumero: string;
  equipIdent: string;
  data: string;
  horimetro: string;
...
export async function buildReportDocx(params: MaintenanceReportParams) {
    ...
    sections: [
      {
        ...
        children: [
          ...
          new Paragraph({ children: [new TextRun("")] }),
          new Paragraph({ children: [new TextRun({ text: "Observações", bold: true, size: 22 })] }),
...
export async function saveMaintenanceReportDocx(params: MaintenanceReportParams & {
  equipamentoId: string;
  histId: string;
  userId?: string | null;
}) {
  let uploadUserId = params.userId ?? null;
  if (!uploadUserId) {
    const { data, error } = await supabase.auth.getUser();
    if (error || !data.user) throw new Error("Não autenticado");
    uploadUserId = data.user.id;
  }

  const blob = await buildReportDocx(params);
  const { data: prev, error: prevErr } = await supabase
    .from("equipamento_fotos")
    .select("id, storage_path")
    .eq("manutencao_historico_id", params.histId)
  ...
  const path = `${params.equipamentoId}/hist-${params.histId}/relatorio-${Date.now()}.docx`;
  const { error: upErr } = await supabase.storage
    .from("equipamento-fotos")
    .upload(path, blob, { contentType: WORD_MIME, upsert: true });
  if (upErr) throw upErr;

  const { error: insErr } = await supabase.from("equipamento_fotos").insert({
    equipamento_id: params.equipamentoId,
    manutencao_historico_id: params.histId,
    storage_path: path,
    uploaded_by: uploadUserId,
    caption: `${REPORT_TAG} Relatório de manutenção ${params.data}.docx`,
  });
...
export function getWordViewerUrl(fileUrl: string) {
