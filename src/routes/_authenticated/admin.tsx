...
function AdminPage() {
  ...
  if (!isAdmin) {
    return <div className="p-6 text-center text-sm text-muted-foreground">Acesso restrito a administradores.</div>;
  }
  return (
...
function NewEquipamento() {
  ...
  const [f, setF] = useState({ numero: "", identificacao: "", placa: "", ano: "", localizacao: "", cl: "" });
  ...
  return (
    <Card className="p-4 space-y-3">
      <h3 className="font-semibold text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> Novo equipamento</h3>
      <div className="grid grid-cols-2 gap-3">
        <div><Label className="text-xs">Nº *</Label><Input value={f.numero} onChange={(e) => setF({ ...f, numero: e.target.value })} placeholder="RE-14" /></div>
        <div><Label className="text-xs">Classe</Label><Input value={f.cl} onChange={(e) => setF({ ...f, cl: e.target.value })} /></div>
      </div>
      <div><Label className="text-xs">Identificação</Label><Input value={f.identificacao} onChange={(e) => setF({ ...f, identificacao: e.target.value })} /></div>
      <div className="grid grid-cols-2 gap-3">
        <div><Label className="text-xs">Placa</Label><Input value={f.placa} onChange={(e) => setF({ ...f, placa: e.target.value })} /></div>
        <div><Label className="text-xs">Ano</Label><Input value={f.ano} onChange={(e) => setF({ ...f, ano: e.target.value })} /></div>
      </div>
      <div><Label className="text-xs">Localização</Label><Input value={f.localizacao} onChange={(e) => setF({ ...f, localizacao: e.target.value })} /></div>
      <Button onClick={() => create.mutate()} disabled={create.isPending} className="w-full">
        {create.isPending ? "Criando..." : "Criar equipamento"}
      </Button>
      <p className="text-[11px] text-muted-foreground">Os demais campos podem ser preenchidos depois na tela do equipamento.</p>
    </Card>
  );
}
