...
type Equip = {
  id: string; item: number | null; cl: string | null; numero: string;
  cartao_ticket: string | null; identificacao: string | null; afericao_taco: string | null;
  placa: string | null; ano: string | null; localizacao: string | null;
  operador_contato: string | null; telefone: string | null; cnh: string | null;
  data_ultima_revisao: string | null; u_revisao: number | null; h_revisao: number | null;
  ...
  const { data: fotos } = useQuery({
    ...
    queryFn: async () => {
      ...
      const withUrl = await Promise.all((data ?? []).map(async (f) => {
        const { data: signed } = await supabase.storage.from("equipamento-fotos")
    ...
    <div className="px-3 py-3 max-w-md mx-auto w-full space-y-3">
      ...
      <Card className="p-4 space-y-3 border-accent/40">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Horímetro atual">
            <Input type="number" inputMode="decimal" value={form.horimetro_atual ?? ""}
              onChange={(e) => setForm({ ...form, horimetro_atual: e.target.value === "" ? null : Number(e.target.value) })} />
          </Field>
          <Field label="Data">
            <Input type="date" value={form.data_horimetro_atual ?? ""}
      ...
      <Card className="p-4 space-y-3">
        <h3 className="font-semibold text-sm flex items-center gap-2">
          ...
          Dados do equipamento {ro && <span className="text-[11px] font-normal text-muted-foreground">(somente leitura)</span>}
        </h3>

        <div className="grid grid-cols-2 gap-3">
          <Field label="Nº"><Input value={form.numero ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, numero: e.target.value })} /></Field>
          <Field label="Classe"><Input value={form.cl ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, cl: e.target.value })} /></Field>
          <Field label="Placa"><Input value={form.placa ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, placa: e.target.value })} /></Field>
          <Field label="Ano"><Input value={form.ano ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, ano: e.target.value })} /></Field>
        </div>

        <Field label="Identificação"><Input value={form.identificacao ?? ""} readOnly={ro}
          onChange={(e) => setForm({ ...form, identificacao: e.target.value })} /></Field>

        <Field label="Cartão / Ticket"><Input value={form.cartao_ticket ?? ""} readOnly={ro}
          onChange={(e) => setForm({ ...form, cartao_ticket: e.target.value })} /></Field>

        <Field label="Localização"><Input value={form.localizacao ?? ""} readOnly={ro}
          onChange={(e) => setForm({ ...form, localizacao: e.target.value })} /></Field>

        <div className="grid grid-cols-2 gap-3">
          <Field label="Operador / contato"><Input value={form.operador_contato ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, operador_contato: e.target.value })} /></Field>
          <Field label="Telefone"><Input value={form.telefone ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, telefone: e.target.value })} /></Field>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <Field label="CNH"><Input value={form.cnh ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, cnh: e.target.value })} /></Field>
          <Field label="Aferição taco"><Input value={form.afericao_taco ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, afericao_taco: e.target.value })} /></Field>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <Field label="Data últ. revisão"><Input type="date" value={form.data_ultima_revisao ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, data_ultima_revisao: e.target.value || null })} /></Field>
          <Field label="U. revisão"><Input type="number" value={form.u_revisao ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, u_revisao: e.target.value === "" ? null : Number(e.target.value) })} /></Field>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <Field label="H. revisão"><Input type="number" value={form.h_revisao ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, h_revisao: e.target.value === "" ? null : Number(e.target.value) })} /></Field>
          <Field label="Próx. revisão (h)"><Input type="number" value={form.proxima_revisao_horimetro ?? ""} readOnly={ro}
            onChange={(e) => setForm({ ...form, proxima_revisao_horimetro: e.target.value === "" ? null : Number(e.target.value) })} /></Field>
        </div>
